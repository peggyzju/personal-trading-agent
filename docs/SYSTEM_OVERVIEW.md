...
>   - **排名 = 单因子,零权重**:过 5 条趋势硬门 → 过的按 `momentum_3m`(=63交易日价格涨幅%)降序。tech_score/ai_score/板块boost **均不参与排名**(v7残留/advisory)。
>   - **动量窗口=60d(3月)的决策(2026-06-29)**:稳健性扫 40/60/120d × 8/10/15仓 = 9/9 赢 SPY。**纯回测 120d(6月)在每档都最高**(199/186/145% vs 60d 172/150/107%),但仍选 60d,理由:①匹配短线5-10天持仓(120d反应慢、换手少,违背定位)②窗口越长越吃幸存者偏差(120d领先可能虚高)③9/9都赢=对窗口不敏感,不追单一最优防过拟合。**用户确认保持3月**。未来若要换需先做"去幸存者偏差"票池重测。
...
>
> **⚠️ 诚实评估(2026-06-29,加 QQQ 对比后):v8 稳赢标普(SPY),但跑输纳指(QQQ)。**
> 近3年(2023-2026):v8 +154% / **QQQ +172%** / SPY +100%(回撤 -19/-23/-19%)。v8 只在 2024/2026 赢 QQQ,2023/2025 输。
> 且 v8 数字被幸存者偏差高估(QQQ 是干净 ETF)→ 真实差距更大,**v8 大概率连平 QQQ 都难**。
> 原因:QQQ 本身就是科技动量集中,与 v8 选票高度重叠 —— v8 在跟一个免费/零换手/零滑点的版本竞争。
> **结论**:v8 是扎实的系统化动量策略(赢 SPY、可验证、控风险),但"无脑买 QQQ"是它没法明确战胜的强基准。
> 跑赢市场极难,被动指数是很强的对手 —— v8 值得跑,但别期望碾压 QQQ。
