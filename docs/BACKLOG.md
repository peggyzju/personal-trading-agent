...
- **退出机制近最优,别动**:三次退出/止损类回测全被否决 —— 收紧止损(稳健有害/whipsaw)· stale 死钱止损(不稳健 + 测不了轮动)· 放宽追踪(年份依赖)。改绩效的杠杆在**买(选股)**不在卖。
...
- 🟡 **资金利用率 ~60%**(提高仓位需谨慎):BULL 满 10 仓仅部署约 62%(两条上限叠加:max_positions×每仓 risk/止损/8%封顶)。策略未验证盈利前加仓 = 放大风险;先确认有稳定正收益,再谈提高 max_positions 或每仓上限。
